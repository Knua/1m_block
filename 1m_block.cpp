...
bool Data_checking(u_char * packet, uint32_t start, uint32_t end){
	...
	strncmp((const char *)(packet + start), "OPTIONS", 7) == 0) {
		...
		if(ptr != NULL){
			...
			string now_packet_host = save_string;
			if(one_m_block.find(now_packet_host) != one_m_block.end()){
